...
- **hwcodec / build.rs**: 호스트 cfg(`#[cfg(windows)]`)로 win.cpp/windows.cc를 android에 컴파일 → `if target_os == "windows"` 로 변경(이미 적용). hwcodec 패치가 안 먹으면 `target\...\{build,.fingerprint}\hwcodec-*` 삭제 후 재빌드.
